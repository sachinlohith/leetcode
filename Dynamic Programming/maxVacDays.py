...
class Solution(object):
    def maxVacationDays(self, flights, days):
        ...
        :rtype: int
        Can make by with just O(k) space where k = number of weeks, since we only need current week and next week information
        """
        if not days or not flights:
            return 0
        dp = [[0 for _ in range(len(days[0]) + 1)] for _ in range(len(days))]
        for week in xrange(len(days[0]) - 1, -1, -1):
            for city in xrange(len(days)):
                dp[city][week] = days[city][week] + dp[city][week + 1]
                for dest in xrange(len(days)):
